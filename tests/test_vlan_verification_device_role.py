...
"""
-> Common Setup Class:
Connects to all devices defined in the testbed YAML file with role 'leaf'.
...
class CommonSetup(aetest.CommonSetup):
    ...
    def connect_to_devices(self, testbed):
        """Connect to all devices with role 'leaf' in the testbed."""
        self.parent.parameters['devices'] = {}
        for device_name, device in testbed.devices.items():
            if getattr(device, 'role', None) == 'leaf':  # Connect only to devices with role 'leaf'
                try:
                    device.connect()
...
-> setup Method:
Attempts to get a specific device from the list of connected devices.
...
class VLANVerificationTestcase(aetest.Testcase):
    ...
    def setup(self, testbed):
        """ Connect to the first available leaf device and load VLAN information """
        leaf_devices = [device for device in testbed.devices.values() if getattr(device, 'role', None) == 'leaf']
        if not leaf_devices:
            self.failed("No leaf devices found in the testbed.")
        self.device = leaf_devices[0]
        try:
